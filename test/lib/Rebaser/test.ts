import {renderSync as nodeSassRenderSync} from "node-sass";
...
const {renderSync: dartSassRenderSync} = require("dart-sass");
...
tape("Rebaser", (test) => {
  for (let [renderer, renderSync] of [['node-sass', nodeSassRenderSync], ['dart-sass', dartSassRenderSync]]) {
    ...
    test.test(renderer, (test) => {
      ...
      test.test("supports well-formed map", (test) => {
        ...
        rebaser.rebase(sassRenderResult.css)
          .then(result => {
            test.equal(cleanCSS(result.css.toString()), cleanCSS(`@font-face {
  font-family: Foo;
...
`));
      ...
      test.test("bypasses remote, absolute and hash-only paths", (test) => {
        ...
        rebaser.rebase(sassRenderResult.css)
          .then(result => {
            test.equal(cleanCSS(result.css.toString()), cleanCSS(`.foo {
